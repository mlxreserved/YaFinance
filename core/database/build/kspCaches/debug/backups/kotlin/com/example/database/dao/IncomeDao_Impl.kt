...
public class IncomeDao_Impl(
  ...
  init {
    ...
    this.__insertAdapterOfIncomeEntity = object : EntityInsertAdapter<IncomeEntity>() {
      protected override fun createQuery(): String =
          "INSERT OR REPLACE INTO `incomes` (`localId`,`server_id`,`account_id`,`category_id`,`is_deleted`,`is_awaiting_dispatch_income`,`amount`,`transaction_date`,`comment`) VALUES (nullif(?, 0),?,?,?,?,?,?,?,?)"
      ...
      protected override fun bind(statement: SQLiteStatement, entity: IncomeEntity) {
        ...
        val _tmp_1: Int = if (entity.isIncomeAwaiting) 1 else 0
  ...
  public override suspend fun insertIncome(income: IncomeEntity): Long = performSuspending(__db,
      false, true) { _connection ->
    val _result: Long = __insertAdapterOfIncomeEntity.insertAndReturnId(_connection, income)
    _result
  }

  public override suspend fun insertAllIncomes(income: List<IncomeEntity>): List<Long> =
      performSuspending(__db, false, true) { _connection ->
    val _result: List<Long> = __insertAdapterOfIncomeEntity.insertAndReturnIdsList(_connection,
        income)
    _result
  }

  public override suspend fun getNotDeletedIncomes(startDate: String?, endDate: String?):
      List<IncomeFullInfo> {
    val _sql: String = """
        |SELECT *
        |        FROM incomes 
    ...
    return performSuspending(__db, true, false) { _connection ->
      ...
      try {
        ...
        val _columnIndexOfIsIncomeAwaiting: Int = getColumnIndexOrThrow(_stmt,
            "is_awaiting_dispatch_income")
        ...
        val _result: MutableList<IncomeFullInfo> = mutableListOf()
        while (_stmt.step()) {
          val _item: IncomeFullInfo
          val _tmpIncome: IncomeEntity
          val _tmpLocalId: Int
          ...
          val _tmpIsIncomeAwaiting: Boolean
          val _tmp_1: Int
          _tmp_1 = _stmt.getLong(_columnIndexOfIsIncomeAwaiting).toInt()
          _tmpIsIncomeAwaiting = _tmp_1 != 0
          ...
          _tmpIncome =
              IncomeEntity(_tmpLocalId,_tmpServerId,_tmpAccountId,_tmpCategoryId,_tmpIsDeleted,_tmpIsIncomeAwaiting,_tmpAmount,_tmpTransactionDate,_tmpComment)
          ...
          _item = IncomeFullInfo(_tmpIncome,_tmpAccount,_tmpCategory)
          _result.add(_item)
        }
        _result
      } finally {
  ...
  public override suspend fun getIncomesAwaitingDispatch(): List<IncomeFullInfo> {
    val _sql: String = """
        |SELECT * 
        |        FROM incomes
        |        JOIN accounts ON incomes.account_id = accounts.accountId
  ...
  public override suspend fun getIncomeById(localId: Int): IncomeFullInfo {
    val _sql: String = """
        |SELECT * 
        |        FROM incomes
        |        JOIN accounts ON incomes.account_id = accounts.accountId
    ...
    return performSuspending(__db, true, false) { _connection ->
      ...
      try {
        ...
        if (_stmt.step()) {
          ...
          _tmpIsIncome = _tmp_3 != 0
          _tmpCategory = CategoryEntity(_tmpCategoryId_1,_tmpCategoryName,_tmpEmoji,_tmpIsIncome)
          _result = IncomeFullInfo(_tmpIncome,_tmpAccount,_tmpCategory)
        } else {
          error("The query result was empty, but expected a single row to return a NON-NULL object of type <com.example.database.entity.income.IncomeFullInfo>.")
        }
        _result
      } finally {
        _stmt.close()
  ...
  public override suspend fun updateIncomeByLocalId(
    localId: Int,
    serverId: Int?,
    amount: String,
    comment: String?,
    ...
    val _sql: String =
        "UPDATE incomes SET is_awaiting_dispatch_income = 1, account_id = ?, category_id = ?, transaction_date = ?, server_id = ?, amount = ?, comment = ? WHERE localId = ?"
  ...
  public override suspend fun updateIncomeAwaitingStatus(localId: Int, serverId: Int?): Int {
    val _sql: String =
        "UPDATE incomes SET is_awaiting_dispatch_income = 0, server_id = ? WHERE localId = ?"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun deleteIncome(localId: Int, serverId: Int?): Int {
    val _sql: String =
        "UPDATE incomes SET is_deleted = 1, is_awaiting_dispatch_income = IIF(? IS NULL, 0, 1) WHERE localId = ?"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
